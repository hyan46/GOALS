rq.no.cross_insample = function(y, x, taus)
{
  require(quantreg)
  ...
  for (i in 1:p)
  {
    ...
    scalings[i] = max(x[,i]) # we only need to change the scaling of this code to accommodate the future prediction points.
    x[,i] = x[,i]/scalings[i]
  }
...
rq.no.cross_pred = function(y, x, forecast_horizon, taus)
{
  require(quantreg)
  ...
  for (i in 1:p)
  {
    x[,i] = xtemp[,i] - shifts[i]
    # scalings[i] = max(x[,i]) # we only need to change the scaling of this code to accommodate the future prediction points.
    scalings[i] = forecast_horizon[i] - shifts[i]
    x[,i] = x[,i]/scalings[i]
  }
...
# We need to check rq.fit.sfnc() function in quantreg package (which is based on KoenkerNg(2005))
